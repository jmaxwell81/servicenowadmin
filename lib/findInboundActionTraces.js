...
function findInboundActionTraces(vsysid) {
  ...
  if (b.isValid()) {
    ...
    a.setLimit(3);
    for (a.query(); a.next();) {
      var f = [a.getDisplayValue(), a.sys_updated_by, a.sys_updated_on.getDisplayValue(), a.sys_created_by, a.sys_created_on.getDisplayValue(), g + a.sys_class_name + ".do?sys_id=" + a.sys_id, a.state.getDisplayValue()];
    ...
    a.addEncodedQuery("messageNOT LIKE which does not match Inbound Email Action^messageNOT LIKE' failed^messageLIKEProcessed^ORmessageLIKESkipping^sys_created_onONLast 90 days@javascript:gs.beginningOfLast90Days()@javascript:gs.endOfLast90Days()");
    a.setLimit(100);
